...
function derive(source) {
  ...
  tokens.fontScale = clamp(Number(tokens.fontScale) || 1, .9, 1.15);
...
function applyPreferences(preferences = workingPreferences) {
  ...
  const variables = {
    '--red':tokens.brand,'--red-hover':tokens.brandHover,'--red-active':tokens.brandActive,'--red-soft':tokens.brandSoft,'--red-faint':tokens.brandFaint,
...
function presetHtml() {
  return Object.entries(PRESETS).map(([id,preset]) => {
    ...
    return `<button type="button" class="theme-preset-card ${active?'active':''} ${exclusive?'theme-preset-card-exclusive':''}" data-theme-preset="${id}" aria-pressed="${active}">${exclusive?'<em class="theme-exclusive-badge">ОФИЦИАЛЬНЫЙ</em>':''}<span class="theme-preset-swatches"><i style="--swatch:${tokens.brand}"></i><i style="--swatch:${tokens.page}"></i><i style="--swatch:${tokens.surface}"></i><i style="--swatch:${tokens.sidebar}"></i></span><span class="theme-preset-copy"><strong>${escapeHtml(preset.short)}</strong><small>${escapeHtml(preset.description)}</small></span></button>`;
  }).join('');
}
...
function captureThemeScrollState() {
  const presetPanel = document.querySelector('.theme-preset-panel');
  const controls = document.getElementById('themeStudioControls');
  return {
    presetTop: presetPanel?.scrollTop || 0,
    presetLeft: presetPanel?.scrollLeft || 0,
...
function restoreThemeScrollState(state) {
  ...
  const restore = () => {
    const presetPanel = document.querySelector('.theme-preset-panel');
    const controls = document.getElementById('themeStudioControls');
    if (presetPanel) { presetPanel.scrollTop = state.presetTop; presetPanel.scrollLeft = state.presetLeft; }
...
function bind() {
  ...
  backdrop?.addEventListener('input',event=>{
    ...
    if(rangeKey==='fontScale'){const value=clamp(Number(event.target.value)/100,.9,1.15);document.getElementById('themeFontScaleValue').textContent=Math.round(value*100)+'%';updateToken(rangeKey,value);}
  ...
  backdrop?.addEventListener('change',event=>{
    const key=event.target.dataset.themeSetting;if(key){updateToken(key,event.target.value);if(key==='mode')render();}
  ...
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&isOpen()){event.preventDefault();event.stopImmediatePropagation();close();}},true);
}
function init() {
  if (ready) return;
